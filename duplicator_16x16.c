...
void setup_duplicator_graphics(void) {
    ...
    memcpy(CHARSET_MEM + ('#' * 8), &duplicator_graphics[0], 8);   // Wall
    memcpy(CHARSET_MEM + ('p' * 8), &duplicator_graphics[8], 8);   // Player
    memcpy(CHARSET_MEM + ('*' * 8), &duplicator_graphics[16], 8);  // Crate
    memcpy(CHARSET_MEM + ('k' * 8), &duplicator_graphics[24], 8);  // Key
    memcpy(CHARSET_MEM + ('d' * 8), &duplicator_graphics[32], 8);  // Door Closed
    memcpy(CHARSET_MEM + ('D' * 8), &duplicator_graphics[40], 8);  // Door Open
    memcpy(CHARSET_MEM + ('e' * 8), &duplicator_graphics[48], 8);  // Enemy
    memcpy(CHARSET_MEM + ('?' * 8), &duplicator_graphics[56], 8);  // Hole A
    memcpy(CHARSET_MEM + ('!' * 8), &duplicator_graphics[64], 8);  // Hole B
    memcpy(CHARSET_MEM + ('b' * 8), &duplicator_graphics[72], 8);  // Plate A
    memcpy(CHARSET_MEM + ('c' * 8), &duplicator_graphics[80], 8);  // Plate B
    memcpy(CHARSET_MEM + ('g' * 8), &duplicator_graphics[88], 8);  // Gate A
    memcpy(CHARSET_MEM + ('h' * 8), &duplicator_graphics[96], 8);  // Gate B
    memcpy(CHARSET_MEM + ('@' * 8), &duplicator_graphics[104], 8); // Exit A
    memcpy(CHARSET_MEM + (':' * 8), &duplicator_graphics[112], 8); // Exit B
    memcpy(CHARSET_MEM + (';' * 8), &duplicator_graphics[120], 8); // Exit C
    memcpy(CHARSET_MEM + ('.' * 8), &duplicator_graphics[128], 8); // Floor
